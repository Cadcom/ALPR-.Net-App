...
namespace ALPR.Detection
{
    ...
    public class OcrResult
    {
        ...
        public float MinConfidence => Details.Count > 0 ? Details.Min(x => x.Confidence) : 0;
        public bool IsSecure => Details.All(x => x.Confidence >= 0.85f);
    }

    public class TitanArmorV6Absolute : IDisposable
    {
        ...
        private readonly float _blankThreshold = 0.1f;

        public TitanArmorV6Absolute(string modelPath, bool useGpu = false)
        {
        ...
        private OcrResult DecodeDetailed(float[] flatOutput)
        {
            ...
            // 1. Olasılıkları Hesapla (Softmax + Temperature)
            ...
            // 2. Best Path Bul (Blank Thresholding dahil)
            ...
            // 3. Karakterleri Grupla ve Ortalama Olasılık Al
